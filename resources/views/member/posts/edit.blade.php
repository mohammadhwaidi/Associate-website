...
@section('content2')
    <div class="container">
        ...
        <form method="POST" action="{{ route('member.posts.update', ['id' => $post->id]) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <a href="{{ route('member.posts.index') }}" class="btn btn-secondary">Cancel</a> <!-- Cancel button -->
        </form>
    </div>
@endsection
