...
@section('content2')
    <div class="container">
        ...
        <form method="POST" action="{{ route('member.activities.update', ['id' => $activity->id]) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <a href="{{ route('member.activities.index') }}" class="btn btn-secondary">Cancel</a>
        </form>
    </div>
@endsection
