                ...
                <div class="col-md-4 mb-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <a href="{{ route('admin.posts.edit', ['id' => $post->id]) }}" class="btn btn-primary mr-2">Edit</a>
                            <form action="{{ route('admin.posts.destroy', ['id' => $post->id]) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger mr-2">Remove</button>
                            </form>
                            <a href="{{ route('admin.posts.show', ['id' => $post->id]) }}" class="btn btn-info">Show</a>
                        </div>
                    </div>
                </div>
            @endforeach
